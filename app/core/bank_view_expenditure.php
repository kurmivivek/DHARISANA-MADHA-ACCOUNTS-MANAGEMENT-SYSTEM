			...
			<div class="panel panel-primary" >
				...
				<div class="panel-body">
					...
					<div class="col-md-offset-4 col-md-5">
						<h3 style="text-align:center"><strong>Expenditure records for <?php echo $monthName.",".$year;?></strong></h3>
    ...
    data: {
        ...
        datasets: [{
            ...
            data: [<?php for ($i=1;$i<=$noOfDays;$i++){
            			if($i<9)
						$i=sprintf("%02d", $i);
            			$income = $db->querySingle("SELECT SUM(amount) FROM record WHERE category='church' and type='expenditure' and strftime('%Y-%m-%d',date)='".$year."-".$month."-".$i."'") or ($income=0);
            			echo $income.',';
            		}
            ?>],
        },
        {
            ...
            data: [<?php for ($i=1;$i<=$noOfDays;$i++){
            			if($i<9)
						$i=sprintf("%02d", $i);
            			$income = $db->querySingle("SELECT SUM(amount) FROM record WHERE category='school' and type='expenditure' and strftime('%Y-%m-%d',date)='".$year."-".$month."-".$i."'") or ($income=0);
            			echo $income.',';
            		}
            ?>],
        }]
