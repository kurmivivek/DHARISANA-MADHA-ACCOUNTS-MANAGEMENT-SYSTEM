...
<?php include 'menu.php'; ?>
        <div class="col-md-12 ">
			<div class="panel panel-primary" >
				<div class="panel-heading">
					<h3><strong>Add School Income</strong></h3>
				</div>
				<div class="panel-body">
					<div id="result"></div>
					<form class="form-horizontal col-md-12 col-sm-12" role="form" id="schoolIncomeForm" autocomplete="off">
						  ...
						  <div class="col-md-12">			
							  <div class="form-group">
								  ...
								  <button type="reset" class="btn-lg btn-primary" onclick="$('#schoolIncomeForm').data('formValidation').resetForm();"><span class="glyphicon glyphicon-repeat"></span> Clear Form</button>
								</div>
								<div class="col-md-4">
					...
					<div class="col-md-offset-2 col-md-9">
						...
						$result = $db->query("SELECT *,strftime('%d-%m-%Y',date) as date,strftime('%d-%m-%Y %H:%M',timestamp) as entry_date FROM (SELECT * FROM record WHERE category='school' and type='income' ORDER BY id DESC LIMIT 5) ORDER BY id ASC") or die("Query failed");
						while ($row = $result->fetchArray())
						{
						echo "<tr><td>{$row['date']}</td><td>{$row['name']}</td><td>{$row['receipt_no']}</td><td class='amount'>".formatInIndianStyle(number_format((float)$row['amount'], 2, '.', ''))."</td><td>{$row['ledger_page_no']}</td><td>{$row['operator_name']}</td><td>{$row['entry_date']}</td><td><a href='edit_record.php?id={$row['id']}' class='btn btn-info' role='button'>Edit</a></td></tr>";
						}
						?>
						</tbody>
						</table>
					...
					<div class="col-md-offset-2 col-md-8">
						<div class="col-md-6">
							...
							<?php 
							$income = $db->querySingle("SELECT SUM(amount) FROM record WHERE category='school' and type='income' and strftime('%Y-%m',date)=strftime('%Y-%m','now','start of month','-1 month','localtime')") or ($income=0);
							$expenditure = $db->querySingle("SELECT SUM(amount) FROM record WHERE category='school' and type='expenditure' and strftime('%Y-%m',date)=strftime('%Y-%m','now','start of month','-1 month','localtime')") or ($Expenditure=0);
						...
						<div class="col-md-12">		
 							...
 							$balance=$income-$expenditure		
 							?>		
 							<span class="col-md-12" style="text-align:center"><strong>School Bank Balance: ₹ </strong><span id="total_balance"><?php echo formatInIndianStyle(number_format((float)$balance, 2, '.', ''));?></span></span>		
 						</div>
					</div>
				</div>
			</div>
	...
	$( "#date" ).datepicker({ dateFormat: 'dd-mm-yy',
		...
		changeYear: true,
		onSelect: function () {
		$('#schoolIncomeForm').formValidation('updateStatus', 'date', 'NOT_VALIDATED')
...
$(document).ready(function()	{
    ...
    $('#schoolIncomeForm').formValidation({
	    	icon: {
        ...
        .on('success.form.fv', function(e) {
            ...
            var jqxhr =$.post("ajax/add_record.php", $form.serialize() , function(result) {
                ...
                if(result['success']){
                	...
                	$("#income").text(income.toLocaleString('en-IN', {minimumFractionDigits:2 ,maximumFractionDigits: 2}));
                	swal({  title: "Success!",   
                	...
                	$('form#schoolIncomeForm').data('formValidation').resetForm();
  		   			$('form#schoolIncomeForm')[0].reset();
  		   			getJoinDate();
  		   		}
               	else
                swal({  title: "Error!",   
    ...
    $("form#schoolIncomeForm").keydown(function(event){//prevent form submit on enter 
	    if( (event.keyCode == 13)  ) {
