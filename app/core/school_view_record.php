        ...
        <div class="col-md-12 ">
			<div class="panel panel-primary" >
				<div class="panel-heading">
					<h3><strong>View School Records</strong></h3>
				</div>
				<div class="panel-body">
					...
					<div class="col-md-offset-1 col-md-10">
						<h3 style="text-align:center"><strong>School records for <?php echo $monthName.",".$year;?></strong></h3>
						...
						$result = $db->query("SELECT *,strftime('%d-%m-%Y',date) as date,strftime('%d-%m-%Y %H:%M',timestamp) as entry_date FROM record WHERE category='school' and strftime('%Y-%m',date)='".$year."-".$month."' ORDER BY id") or die("Query failed");
						while ($row = $result->fetchArray())
						{
							if($row['type']=='income'){
							echo "<tr><td>{$row['date']}</td><td>{$row['name']}</td><td>{$row['receipt_no']}</td><td>{$row['amount']}</td><td></td><td></td><td>{$row['ledger_page_no']}</td><td>{$row['operator_name']}</td><td>{$row['entry_date']}</td><td><a href='edit_record.php?id={$row['id']}' class='btn btn-info' role='button'>Edit</a></td></tr>";
							}
							else{
							echo "<tr><td>{$row['date']}</td><td>{$row['name']}</td><td>{$row['receipt_no']}</td><td></td><td>{$row['amount']}</td><td>{$row['bill_no']}</td><td>{$row['ledger_page_no']}</td><td>{$row['operator_name']}</td><td>{$row['entry_date']}</td><td><a href='edit_record.php?id={$row['id']}' class='btn btn-info' role='button'>Edit</a></td></tr>";
							}
						}
						?>
						<tr class="success">
						...
						<tr class="info">		
 							<td data-toggle="tooltip" data-original-title="Bank balance of all months combined" colspan="10" style="text-align:center"><strong>School Cumulative bank balance ₹ : 	
 							<?php 		
